...
class AImage {
  ...
  ///加载网络图片
  static Widget netImage(String placeholder,
      {String? url, Size? size, BoxFit? fit, Alignment? alignment}) {
    if (url == null || url == "") {
  ...
  ///加载网络图片 四个圆角弧度一样
  static Widget netImageRadiusAll(String placeholder,
      {String? url, Size? size, BoxFit? fit, double? radius}) {
    return ClipRRect(
      borderRadius: BorderRadius.circular(radius ?? 0),
      child: netImage(placeholder, url: url, size: size, fit: fit),
    );
  }

  ///加载网络图片 四角可设置不同弧度
  static Widget netImageRadius(String placeholder,
      {String? url,
      Size? size,
      BoxFit? fit,
      double? topLeft,
  ...
  ///加载网络图片 圆形
  static Widget netImageCircle(String placeholder,
      {String? url, Size? size, BoxFit? fit}) {
    return ClipOval(
      child: netImage(placeholder, url: url, size: size, fit: fit),
    );
  }
