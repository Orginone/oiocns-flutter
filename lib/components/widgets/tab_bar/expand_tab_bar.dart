...
class _TabBarScrollPosition extends ScrollPositionWithSingleContext {
  ...
  final _ExpandTabBarState? tabBar;
...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color? backgroundColor = selected!
        ? Color.lerp(bgColor, unselectedBgColor, animation.value)
        : Color.lerp(unselectedBgColor, bgColor, animation.value);
    final Color? tabBorderColor = selected!
        ? Color.lerp(borderColor, unselectedBorderColor, animation.value)
        : Color.lerp(unselectedBorderColor, borderColor, animation.value);
...
class _TabLabelBar extends Flex {
  ...
  void updateRenderObject(
      BuildContext context, _OTabLabelBarRenderer renderObject) {
...
class _OTabLabelBarRenderer extends RenderFlex {
  _OTabLabelBarRenderer({
...
class ExpandTabBar extends StatefulWidget implements PreferredSizeWidget {
  ExpandTabBar({
    Key? key,
    this.tabNames = const [],
    this.tabs = const [],
    ...
    this.mainAxisAlignment,
        ...
        super(key: key) {
    isCustomTabItem = null != buildTabItem;
    this.buildTabItem = buildTabItem ??
  ...
  /// and the entire [ExpandTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// How the [ExpandTabBar]'s scroll view should respond to user input.
  ///
...
class _ExpandTabBarState extends State<ExpandTabBar> {
  ...
  void initTabs() {
    if ((widget.tabs.isEmpty || widget.tabs.length < widget.tabNames.length)) {
      int index = 0;
      widget.tabs = widget.tabNames
          .map((String tabName) => widget.buildTabItem(
              context,
              tabName,
    ...
    _tabKeys.clear();
    _tabKeys.addAll(widget.tabs.map((Widget tab) => GlobalKey()).toList());
  }
  ...
  @override
  void didUpdateWidget(ExpandTabBar oldWidget) {
  ...
  Widget _buildTabItem(int index, bool selected, TabBarTheme? tabBarTheme) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(10.w),
        border: null == widget.unselectedBorderColor
            ? null
            : Border.all(
                width: 1,
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> wrappedTabs =
      ...
      return _buildTabItem(index, false, tabBarTheme);
    });
    ...
    if (_controller != null) {
      ...
      if (_controller!.indexIsChanging) {
        ...
        Animation<double> animation = _ChangeAnimation(_controller!);
        wrappedTabs[_currentIndex!] = _buildStyledTab(
            widget.isCustomTabItem
                ? _buildTabItem(_currentIndex!, true, tabBarTheme)
        ...
        // int tabIndex = 0;
        // animation = ReverseAnimation(_ChangeAnimation(_controller!));
        // do {
        //   if (tabIndex == _currentIndex) continue;
        //   wrappedTabs[tabIndex] =
        //       _buildStyledTab(wrappedTabs[tabIndex], false, animation);
        // } while (tabIndex++ < widget.tabs.length - 1);
        // int tabIndex = 0;
        // if (_currentIndex! > 0) {
        ...
        wrappedTabs[tabIndex] = _buildStyledTab(
            widget.isCustomTabItem
                ? _buildTabItem(tabIndex, true, tabBarTheme)
                : wrappedTabs[tabIndex],
            true,
            centerAnimation);
        if (_currentIndex! > 0) {
          ...
          do {
            ...
            wrappedTabs[tabIndex] = _buildStyledTab(
                wrappedTabs[tabIndex], false, previousAnimation);
          } while (tabIndex-- > 0);
        }
        if (_currentIndex! < widget.tabs.length - 1) {
          int tabIndex = _currentIndex! + 1;
