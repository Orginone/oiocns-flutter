...
abstract class ITarget {
  ...
  // 子组织类型
  late List<String> subTeamTypes;
  // 可以加入的父组织类型
  late List<String> joinTargetType;
  // 可以查询的组织类型
...
abstract class IFlow {
 ...
 late List<XFlowDefine> defines; 
   /// 获取流程定义列表
   /// @param reload 是否强制刷新 
  Future<List<XFlowDefine>>  getDefines(bool reload); 
  ///查询流程定义绑定项
  ///@param reload 是否强制刷新 
   Future<List<XFlowRelation>> queryFlowRelation(bool reload); 
  ///发布流程定义（包含创建、更新）
  ///@param data 
...
abstract class   IPerson implements ISpace, ITarget {
 ...
 /// 修改密码
   /// @param password 新密码
   /// @param privateKey 私钥
  Future<bool>  resetPassword(password, privateKey);
 /// 查询单位
   /// @param code 单位的信用代码
...
abstract class   IGroup implements ITarget {
 ...
 /// 申请加入集团
   /// @param id 目标Id
   /// @returns 
  Future<ResultType>   applyJoinGroup(String id);
   /// 创建子集团
   /// @param data 子集团基本信息
 Future<IGroup>   createSubGroup(TargetParam data);
 /// 删除子集团
   /// @param id 集团Id
   /// @returns
...
late List<IWorking>  workings;
 ...
 /// 获取工作组
   ...
   Future<List<IWorking>>   getWorkings(bool reload);
  /// 创建子部门
  Future<IDepartment>  createDepartment(TargetParam data);
  /// 创建工作组
  Future<IWorking>  createWorking(TargetParam data);
  /// 删除子部门
 Future<bool>   deleteDepartment(String id);
  /// 删除工作组
  Future<bool>  deleteWorking(String id);
